...
export function AdminPage({ currentUser, onNotice, onError }: AdminPageProps) {
  ...
  const visibleUsers = useMemo(() => {
    ...
    return users.filter((user) =>
      `${user.username} ${user.role}`.toLowerCase().includes(query),
    )
  }, [search, users])
  ...
  async function handleDelete(user: User) {
    ...
    if (!window.confirm(`确定删除用户“${user.username}”及其全部剪贴板数据吗？`)) {
      return
    }
  ...
  return (
    <main className="container space-y-6 py-5 sm:py-8">
        ...
        <div className="space-y-3">
          ...
          <div>
            ...
            <p className="mt-2 max-w-2xl text-sm leading-6 text-muted-foreground sm:text-base">
              在这里管理账号、角色和访问边界。每个账号只能看到自己的剪贴板与设备活动。
            </p>
          </div>
        </div>
          ...
          <CardContent className="pt-5">
            <form className="space-y-4" onSubmit={(event) => void handleCreate(event)}>
              ...
              <div className="rounded-lg bg-muted/60 p-3 text-xs leading-5 text-muted-foreground">
                ...
                <p className="mt-1">
                  密码只以 bcrypt 哈希形式保存，管理员无法读取原密码。公网使用前请启用
                  HTTPS。
                </p>
              </div>
            </form>
        ...
        <Card className="overflow-hidden">
          ...
          <CardContent className="p-4 sm:p-5">
            {loading ? (
              ...
              <EmptyUsers searched={Boolean(search.trim())} />
            ) : (
              <UserList
                users={visibleUsers}
      ...
      <div className="flex items-center gap-2 text-xs text-muted-foreground">
        ...
        当前登录：{currentUser.username} · 管理员权限
      </div>
    </main>
  )
}
...
function UserList({
  ...
  return (
    <div className="space-y-2">
      <div className="hidden grid-cols-[minmax(0,1fr)_7rem_8rem_5rem] gap-3 px-3 pb-1 text-xs text-muted-foreground sm:grid">
        <span>账号</span>
        <span>角色</span>
        <span>创建时间</span>
        <span />
      ...
      {users.map((user) => {
        ...
        return (
          <div
            key={user.id}
            className="grid grid-cols-[minmax(0,1fr)_auto] items-center gap-3 rounded-lg border px-3 py-3 sm:grid-cols-[minmax(0,1fr)_7rem_8rem_5rem]"
          >
...
function AdminMetric({
  ...
  return (
    <div className="rounded-xl border bg-card p-3 shadow-sm sm:p-4">
      <div className="mb-2 flex h-7 w-7 items-center justify-center rounded-lg bg-muted text-foreground">
        {icon}
      </div>
...
function EmptyUsers({ searched }: { searched: boolean }) {
  return (
    <div className="flex min-h-36 flex-col items-center justify-center rounded-lg border border-dashed px-6 text-center">
      <Users className="h-6 w-6 text-muted-foreground" />
      <p className="mt-3 text-sm font-medium">
        {searched ? "没有匹配的账号" : "还没有用户"}
      </p>
      <p className="mt-1 text-xs text-muted-foreground">
        {searched ? "换一个用户名或角色试试" : "从左侧创建第一个独立账号"}
      </p>
    </div>
  )
}
